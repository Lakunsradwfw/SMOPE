#!/usr/bin/env bash
# Causal audit treatment: fixed functional-tangent Split-Lite projection only.
# Example: GPUID=1 bash experiments/cifar-100_audit_functional_projection.sh

DATASET=${DATASET:-cifar-100}
OUTDIR=${OUTDIR:-outputs/${DATASET}/10-task/audit-functional-projection}
GPUID=${GPUID:-1}
REPEAT=${REPEAT:-3}
...
python -u run.py --config configs/cifar-100_prompt_smope.yaml --gpuid "${GPUID}" --repeat "${REPEAT}" --overwrite "${OVERWRITE}" \
  ...
  --enable_causal_audit \
  --experiment_version audit_functional_projection \
  --log_dir "${OUTDIR}"
