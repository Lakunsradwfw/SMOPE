#!/usr/bin/env bash
# v6 Exp3: Exp2 plus live functional-basis conflict adaptive projection.
# Example: GPUID=2 MAX_TASK=5 REPEAT=3 bash experiments/cifar-100_v6_exp3_conflict_topk16.sh
...
python -u run.py --config configs/cifar-100_prompt_smope.yaml --gpuid "${GPUID}" --repeat "${REPEAT}" --overwrite "${OVERWRITE}" \
  ...
  --experiment_version v6_exp3_conflict_topk16 \
  --log_dir "${OUTDIR}/v6-exp3-conflict-topk${ACTIVE_TOPK}"
