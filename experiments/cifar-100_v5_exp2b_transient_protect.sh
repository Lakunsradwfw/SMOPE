# bash experiments/cifar-100_v5_exp2b_transient_protect.sh
# v5 ablation: transient prompt only rescales e_pv protection strength.
...
python -u run.py --config configs/cifar-100_prompt_smope.yaml --gpuid $GPUID --repeat $REPEAT --overwrite $OVERWRITE \
    ...
    --transient_lr 1e-3 --transient_cp_bias_weight 0.0 --transient_protect_scale 1.0 \
    --experiment_version v5_exp2b_transient_protect \
    --log_dir ${OUTDIR}/v5-exp2b-transient-protect-topk${ACTIVE_TOPK}
