# Confirm the task-local expert usage candidate on the full CIFAR-100 10-task setting.
#
# Defaults are conservative for iteration. Increase REPEAT to 5 for a formal run:
#   REPEAT=5 bash experiments/cifar-100_v5_task_usage_max10.sh
...
VERSION="v5_${USAGE_MODE}_usage_max10_${STRICT_LABEL}"
LOG_DIR="${OUTDIR}/v5-${USAGE_MODE}-usage-max10-${STRICT_LABEL}-topk${ACTIVE_TOPK}"
