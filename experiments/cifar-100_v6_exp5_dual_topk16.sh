#!/usr/bin/env bash
# v6 Exp5: complete dual loop: functional conflict protection + transient risk/reward routing.
# Example: GPUID=4 MAX_TASK=10 REPEAT=5 bash experiments/cifar-100_v6_exp5_dual_topk16.sh
...
python -u run.py --config configs/cifar-100_prompt_smope.yaml --gpuid "${GPUID}" --repeat "${REPEAT}" --overwrite "${OVERWRITE}" \
  ...
  --split_lite_adaptive_conflict --split_lite_use_transient_risk \
  --split_lite_adaptive_alpha_max 0.5 --split_lite_conflict_weight 0.6 \
  ...
  --experiment_version v6_exp5_dual_topk16 \
  --log_dir "${OUTDIR}/v6-exp5-dual-topk${ACTIVE_TOPK}"
